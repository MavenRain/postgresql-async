  ...
  "prepared statements" should {
    ...
    "support prepared statement with more than 64 characters" in {
      withHandler {
        handler =>
          ...
          executePreparedStatement(handler, messagesInsert, Array(firstContent, date))
          executePreparedStatement(handler, messagesInsertReverted, Array(date, secondContent))

          val rows = executePreparedStatement(handler, messagesSelectAll).rows.get
    ...
    "raise an exception if the parameter count is different from the given parameters count" in {
      ...
      withHandler {
        handler =>
          executeDdl(handler, messagesCreate)
          executePreparedStatement(handler, messagesSelectOne) must throwAn[InsufficientParametersException]
      }
    ...
    "run two different prepared statements in sequence and get the right values" in {
      ...
      withHandler {
        handler =>
          ...
          foreach(1.until(4)) {
            x =>
              executePreparedStatement(handler, messagesInsert, Array(message, moment))
              executePreparedStatement(handler, insert, Array(otherMoment, otherMessage))

              val result = executePreparedStatement(handler, messagesSelectAll).rows.get
              result.size === x
    ...
    "support prepared statement with Option parameters (Some/None)" in {
      withHandler {
        handler =>
          ...
          executeDdl(handler, messagesCreate)
          executePreparedStatement(handler, messagesInsert, Array(Some(firstContent), None))
          executePreparedStatement(handler, messagesInsert, Array(Some(secondContent), Some(date)))
    ...
    "supports sending null first and then an actual value for the fields" in {
      withHandler {
        handler =>
          ...
          executeDdl(handler, messagesCreate)
          executePreparedStatement(handler, messagesInsert, Array(firstContent, null))
          executePreparedStatement(handler, messagesInsert, Array(secondContent, date))

          val rows = executePreparedStatement(handler, messagesSelectByMoment, Array(null)).rows.get
          rows.size === 0
          ...
          */
          ...
          val rowsWithoutNull = executePreparedStatement(handler, messagesSelectByMoment, Array(date)).rows.get
    ...
    "support prepared statement with escaped placeholders" in {
      withHandler {
        handler =>
          ...
          val rows = executePreparedStatement(handler, messagesSelectEscaped, Array(0)).rows.get
    ...
    "run prepared statement twice with bad and good values" in {
      withHandler {
        handler =>
          ...
          executeDdl(handler, messagesCreate)
          executePreparedStatement(handler, messagesInsert, Array(Some(content), None))
