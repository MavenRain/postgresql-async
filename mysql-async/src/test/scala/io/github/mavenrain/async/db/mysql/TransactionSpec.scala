        ...
        connection =>
          ...
          val future = connection.inTransaction {
            ...
            c.sendPreparedStatement(insert)
             .flatMap( r => connection.sendPreparedStatement(insert))
          }

          awaitFuture(future)

          val result = executePreparedStatement(connection, select).rows.get
          result.size === 2
    ...
    "correctly rollback changes if the transaction raises an exception" in {
      ...
      withConnection {
        connection =>
          ...
          val future = connection.inTransaction {
            c =>
              c.sendQuery(insert).flatMap(r => c.sendQuery(BrokenInsert))
          }
    ...
    "should make a connection invalid and not return it to the pool if it raises an exception" in {
      ...
      withPool {
        pool =>
          ...
          executeQuery(pool, createTable)
          executeQuery(pool, insert)
