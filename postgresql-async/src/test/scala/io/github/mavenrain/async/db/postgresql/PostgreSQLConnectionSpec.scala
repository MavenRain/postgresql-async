...
class PostgreSQLConnectionSpec extends Specification with DatabaseTestHelper {
  ...
  "handler" should {
    ...
    "select rows in the database" in {
      ...
      withHandler {
        handler =>
          ...
          val result = executeQuery(handler, select)
    ...
    "execute a prepared statement" in {
      ...
      withHandler {
        handler =>
          executeDdl(handler, preparedStatementCreate)
          executeDdl(handler, preparedStatementInsert, 1)
          val result = executePreparedStatement(handler, preparedStatementSelect)
    ...
    "execute a prepared statement with parameters" in {
      ...
      withHandler {
        handler =>
          executeDdl(handler, preparedStatementCreate)
          executeDdl(handler, preparedStatementInsert, 1)
          executeDdl(handler, preparedStatementInsert2, 1)
          executeDdl(handler, preparedStatementInsert3, 1)
    ...
    "use RETURNING in an insert statement" in {
      ...
      withHandler {
        connection =>
          executeDdl(connection, preparedStatementCreate)
          val result = executeQuery(connection, preparedStatementInsertReturning)
    ...
    "execute multiple prepared statements" in {
      withHandler {
        handler =>
          executeDdl(handler, preparedStatementCreate)
          for (i <- 0 until 1000)
            executePreparedStatement(handler, preparedStatementInsert)
          ok
      }
    }
    ...
    "insert without return after select" in {
      ...
      withHandler {
        handler =>
          executeDdl(handler, preparedStatementCreate)
          executeDdl(handler, preparedStatementInsert, 1)
          executeDdl(handler, preparedStatementSelect, 1)
          val result = executeQuery(handler, preparedStatementInsert2)
